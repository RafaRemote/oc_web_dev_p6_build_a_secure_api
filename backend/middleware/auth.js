...
module.exports = (req, res, next) => {
  try {                                                             //use try/catch to manage the errors, because multiple can go wrong in these lignes of codes
    ...
    const decodedToken = jwt.verify(token, 'RANDOM_TOKEN_SECRET');  //decode the token with jsonwebtoken method: verify: 2 parameters: the token and the secret key. if error, it will be returned in the catch, else return a js object, so we can manipulate it.
